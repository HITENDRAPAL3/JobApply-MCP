...
export class BrowserManager {
        ...
        const fields = await page.evaluate(() => {
            ...
            elements.forEach((el, index) => {
                ...
                if (tagName === 'textarea') {
                    ...
                    const rawType = inputEl.type?.toLowerCase() || 'text';
     ...
     */
    async fillFields(mappings, resumeFilePath) {
        if (!this.page) {
            throw new Error('Browser is not open. Call open_job_portal first.');
        }
        const page = this.page;
     ...
     */
    async inspectAndScreenshot(screenshotName = 'preview') {
        if (!this.page) {
            throw new Error('Browser is not open. Call open_job_portal first.');
        }
        const page = this.page;
        ...
        let detectedSubmitButton = undefined;
     ...
     */
    async submitApplication(userConfirmed) {
        if (!userConfirmed) {
            throw new Error('SECURITY_GATE: Submission blocked. Explicit user confirmation (user_confirmed: true) is required before clicking Submit.');
        }
        if (!this.page) {
            throw new Error('Browser is not open. Call open_job_portal first.');
        }
