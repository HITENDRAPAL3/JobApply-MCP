...
const TOOLS: Tool[] = [
  {
    name: 'parse_resume',
    description:
      'Parses a local PDF resume file and extracts structured contact info (name, email, phone, location, LinkedIn, GitHub, portfolio), recognized skills, and full text.',
    inputSchema: {
      type: 'object',
      properties: {
        resume_path: {
  ...
  },
  {
    ...
    description:
      'Launches a visible Chromium browser window and navigates to the specified job URL. Keeps the browser open so the user can watch progress and handle any login/CAPTCHA prompts.',
    inputSchema: {
      type: 'object',
      properties: {
        job_url: {
  ...
  },
  {
    ...
    description:
      'Detects and clicks common "Apply" / "Apply Now" buttons on job listings if the application form is not yet visible.',
    inputSchema: {
      type: 'object',
      properties: {}
    }
  },
  {
    ...
    description:
      'Scans the active page and returns a structured list of all form inputs, textareas, selects, and file upload fields with their labels, types, required flags, and current values.',
    inputSchema: {
      type: 'object',
      properties: {}
    }
  },
  {
    ...
    description:
      'Populates the application form fields with given mappings and attaches the resume PDF file to the file input.',
    inputSchema: {
      type: 'object',
      properties: {
        mappings: {
  ...
  },
  {
    ...
    description:
      'Captures a full-page screenshot of the application and returns a summary of filled vs empty required fields, plus status of the submit button for user review.',
    inputSchema: {
      type: 'object',
      properties: {
        screenshot_name: {
  ...
  },
  {
    ...
    description:
      'Submits the job application by clicking the final submit button. STRICT SAFETY GATE: user_confirmed MUST be true. This tool must ONLY be called after receiving explicit confirmation from the user in chat.',
    inputSchema: {
      type: 'object',
      properties: {
        user_confirmed: {
          type: 'boolean',
          description:
            'Must be true. Confirmation from user explicitly permitting submission.'
        }
      },
  ...
  try {
    switch (name) {
      ...
      case 'fill_application_fields': {
        const mappings = (args?.mappings as any[]) || [];
